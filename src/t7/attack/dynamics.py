...
to a :class:`~t7.records.Rollout`. This is the deferred-to-GPU swap point
...
* :class:`SyntheticDynamics` — a deterministic kinematic integrator for local
  ...
  :class:`~t7.metric.state.SyntheticStateAdapter` schema so metric A scores them
...
from t7.records import ACTION_DIM, Rollout, RolloutStep
...
class SyntheticDynamics:
    ...
    ``privileged_state`` dict the :class:`~t7.metric.state.SyntheticStateAdapter`
