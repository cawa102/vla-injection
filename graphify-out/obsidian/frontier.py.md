...
## Connections
- [[(ASR, evasion) Pareto-frontier geometry for §4b-(II) — pure and model-free.  The]] - `rationale_for` [EXTRACTED]
- [[.__post_init__()_2]] - `defined_in` [EXTRACTED]
- [[.__post_init__()_1]] - `defined_in` [EXTRACTED]
- [[A canonical ``(ASR, evasion)`` Pareto frontier.      ``points`` must be ordered]] - `defined_in` [EXTRACTED]
- [[ASR achievable at a fixed ``evasion`` level, linearly interpolated.      Outside]] - `defined_in` [EXTRACTED]
- [[Frontier]] - `contains` [EXTRACTED]
- [[FrontierPoint]] - `contains` [EXTRACTED]
- [[One achievable ``(ASR, evasion)`` operating point of the attacker.      Attribut]] - `defined_in` [EXTRACTED]
- [[Return the non-dominated set, deduplicated and ordered by evasion ascending.]] - `defined_in` [EXTRACTED]
- [[True iff ``q`` Pareto-dominates ``p`` (= on both axes,  on at least one).]] - `defined_in` [EXTRACTED]
- [[_dominates]] - `defined_in` [EXTRACTED]
- [[_dominates()]] - `contains` [EXTRACTED]
- [[asr_at_evasion()]] - `contains` [EXTRACTED]
- [[pareto_frontier()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Pareto_Frontier__ASR

## 📄 Source

`src/evasion_tax/attack/frontier.py`
