---
source_file: "docs/core/goal-action-consistency-detector.md"
type: "document"
...
# goal-action-consistency-detector.md

## Connections
- [[AlignSentinel attention-map detector (arXiv2602.13597)]] - `defined_in` [EXTRACTED]
- [[Goal-Action Consistency Detector (understanding doc)]] - `defined_in` [EXTRACTED]
- [[Indirect prompt-injection threat model (trusted operator goal)]] - `defined_in` [EXTRACTED]
- [[Instruction Hierarchy (arXiv2404.13208)]] - `defined_in` [EXTRACTED]
- [[OpenVLA paper (arXiv2406.09246)]] - `defined_in` [EXTRACTED]
- [[SABER fluent NL injection attack (arXiv2603.24935)]] - `defined_in` [EXTRACTED]
- [[Task Drift — Activation Deltas (arXiv2406.00799)]] - `defined_in` [EXTRACTED]
- [[The Embodiment Evasion Tax (EET)]] - `defined_in` [EXTRACTED]
- [[Trusted-reference coarsening ladder]] - `defined_in` [EXTRACTED]
- [[actalign  Do What You Say (arXiv2510.16281)]] - `defined_in` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/L1_Probe_Literature

## 📄 Source

`docs/core/goal-action-consistency-detector.md`

# Goal-Action Consistency Detection for Instruction-Injected VLA Policies
...
> threat-model foundation. Operational source of truth = [`execution-playbook.md`](./execution-playbook.md)
> — **§3a (reframe), §2 (tiered roadmap), §4b (what to implement), H6 (hypothesis)**. (*The Embodiment Evasion
> Tax* is the dissertation title.)

> **Status:** Theme **agreed 2026-05-30**. Phase: Scope → **Design** (CLAUDE.md §3).
> **This document** consolidates the project-specific reasoning from the theme-scoping session into one
> self-contained place, to drive the next decisions about the project's direction. Broader landscape, gap map,
...
## 2. Why this theme (decision rationale)
...
- **vs T5 (action-tokenization defense):** T5's mechanism is tied to OpenVLA's discrete 256-bin head and
  **breaks on OpenVLA-OFT** (continuous L1 head) — model lock-in. The Embodiment Evasion Tax is **head-agnostic**.
- **vs T3 (clean-label poisoning):** T3 is the lowest-build-risk measurement study but less salient; the Embodiment Evasion Tax
...
- **vs T9 (persistent-backdoor defense):** T9 scored higher on raw novelty/viva-defensibility and has an
  open lane, but is **training-heavy** (inject backdoor + simulate user fine-tune). the project's attack side is
  **training-free** (inference-time GCG) → lighter compute. **The user chose this theme.**

**Honest caveat carried forward:** the project's novelty is *narrow and in a contested lane* (see §6). The framing
...
> **The detector's job is exactly to bridge** "attacker reached an arbitrary low-level action" → "that behavior violates
...
- **Adjacent prior work [verified 2026-05-31 via arXiv abstracts; all 4 Codex-flagged IDs resolved
  correctly].** None scoop the project's *embodied, action-level* contribution, but they **narrow the novelty claim**:
  ...
  - **Task Drift — "Get my drift? Catching LLM Task Drift with Activation Deltas"** (`2406.00799`, Abdelnabi
    ...
    attacker-aware, **text LLM only**. *This work differs:* embodied VLA, **behavioural goal↔action-window** signal
  ...
  - **Instruction Hierarchy** (`2404.13208`, Wallace et al., **OpenAI**, 2024): a **training method** to
    prioritise privileged over injected instructions, **text LLM only**. *This work differs:* a **runtime detector**
  ...
  - **AlignSentinel** (`2602.13597`, Jia, Wang, Wang, Xiang, **Gong**, 2026): **closest prior work** — a
    ...
    **explicitly reduces false positives** on benign instruction-bearing inputs — but **text LLM**,
    instruction-classification level. *This work differs:* VLA **action-level** goal-consistency, RoboGCG
  ...
  - **SABER** (`2603.24935`, Wu et al., UMD/Manocha, 2026): a **natural-language** instruction-edit **attack**
    ...
    not detector → does not scoop this work.* **Opportunity:** a ready-made **fluent / low-perplexity** VLA injection
...
1. **RoboGCG — "Adversarial Attacks on Robotic Vision Language Action Models"** — `arXiv:2506.03350`
   ...
   *The attack we reproduce.* **Full text saved locally: `docs/references/2506.03350-robogcg-adversarial-attacks-vla.pdf`
   ...
   defense problem is **open**, motivating the project's FP-calibrated behavioral detector (see §5/§6 baselines).
