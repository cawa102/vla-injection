...
## Connections
- [[.__post_init__()_8]] - `defined_in` [EXTRACTED]
- [[.to_privileged_state()]] - `defined_in` [EXTRACTED]
- [[.to_privileged_state()_1]] - `defined_in` [EXTRACTED]
- [[Build a class`PrivilegedState` from a plain fixture ``dict``.      Expected ke]] - `defined_in` [EXTRACTED]
- [[Coerce a length-3 numeric sequence to a tuple of 3 floats.      Args         va]] - `defined_in` [EXTRACTED]
- [[Map a fixture dict to a class`PrivilegedState`.          Args             raw]] - `defined_in` [EXTRACTED]
- [[Map a raw envground-truth snapshot to a class`PrivilegedState`.]] - `defined_in` [EXTRACTED]
- [[Normalised, env-agnostic ground-truth snapshot the metric reasons over.      Thi]] - `defined_in` [EXTRACTED]
- [[PrivilegedState_1]] - `contains` [EXTRACTED]
- [[PrivilegedState (env-agnostic snapshot)]] - `defined_in` [EXTRACTED]
- [[Seam between a raw env snapshot and the normalised class`PrivilegedState`.]] - `defined_in` [EXTRACTED]
- [[StateAdapter]] - `contains` [EXTRACTED]
- [[StateAdapter (Protocol seam)]] - `defined_in` [EXTRACTED]
- [[SyntheticStateAdapter]] - `contains` [EXTRACTED]
- [[SyntheticStateAdapter_1]] - `defined_in` [EXTRACTED]
- [[Thin, swappable privileged-state adapter (Task 4).  The consistency metric (A) m]] - `rationale_for` [EXTRACTED]
- [[_coerce_position()_1]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Privileged_State_Adapter

## 📄 Source

`src/evasion_tax/metric/state.py`
