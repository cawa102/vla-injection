...
## Connections
- [[._primary_fpr_is_a_target()]] - `defined_in` [EXTRACTED]
- [[._within_unit_interval()]] - `defined_in` [EXTRACTED]
- [[A fully-pinned run configuration (the snapshot logged with every run).]] - `defined_in` [EXTRACTED]
- [[AttackConfig]] - `contains` [EXTRACTED]
- [[Base for every config section immutable + rejects unknown fields.]] - `defined_in` [EXTRACTED]
- [[Calibrationtest manifests (disjointness enforced at eval time).]] - `defined_in` [EXTRACTED]
- [[Config]] - `contains` [EXTRACTED]
- [[Consistency metric (A) settings — the causal prefix-window length ``k``.]] - `defined_in` [EXTRACTED]
- [[DetectorConfig]] - `contains` [EXTRACTED]
- [[EnvConfig]] - `contains` [EXTRACTED]
- [[EvalConfig]] - `contains` [EXTRACTED]
- [[FP-calibration targets — the per-rollout benign false-abort budgets.      ``prim]] - `defined_in` [EXTRACTED]
- [[Flatten a nested dict into ``{dotted.path leaf_value}`` (lists are leaves).]] - `defined_in` [EXTRACTED]
- [[Load and validate a pinned-config YAML file.      Args         path Path to a]] - `defined_in` [EXTRACTED]
- [[MetricConfig]] - `contains` [EXTRACTED]
- [[ModelConfig]] - `contains` [EXTRACTED]
- [[Pinned-config schema + one-variable-diff (Task 9).  A run's parameters are valid]] - `rationale_for` [EXTRACTED]
- [[Return the sorted dotted paths of leaf fields that differ between configs.]] - `defined_in` [EXTRACTED]
- [[SplitManifest]] - `contains` [EXTRACTED]
- [[SplitsConfig]] - `contains` [EXTRACTED]
- [[StrPath]] - `defined_in` [EXTRACTED]
- [[The LIBERO suite + the tasksepisode length a run covers.]] - `defined_in` [EXTRACTED]
- [[The attack arm and its window-scored target budget (decision D2).]] - `defined_in` [EXTRACTED]
- [[The condition matrix to evaluate plus the calibrationtest splits.]] - `defined_in` [EXTRACTED]
- [[The ids on each disjointness axis for one split (calib or test).]] - `defined_in` [EXTRACTED]
- [[The victim policy checkpoint and its action-decoding key.]] - `defined_in` [EXTRACTED]
- [[_Frozen]] - `contains` [EXTRACTED]
- [[_Frozen (immutable, extra-forbid base)]] - `defined_in` [EXTRACTED]
- [[_flatten]] - `defined_in` [EXTRACTED]
- [[_flatten()]] - `contains` [EXTRACTED]
- [[load_config()]] - `contains` [EXTRACTED]
- [[one_variable_diff()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Config_Schema__Immutability

## 📄 Source

`src/evasion_tax/config/schema.py`
