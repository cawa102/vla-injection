...
## Connections
- [[.__post_init__()_9]] - `defined_in` [EXTRACTED]
- [[.action_dim()]] - `defined_in` [EXTRACTED]
- [[.bin_centers()]] - `defined_in` [EXTRACTED]
- [[.bin_to_norm()]] - `defined_in` [EXTRACTED]
- [[.bins()]] - `defined_in` [EXTRACTED]
- [[.decode()]] - `defined_in` [EXTRACTED]
- [[.from_stats()]] - `defined_in` [EXTRACTED]
- [[.token_to_bin()]] - `defined_in` [EXTRACTED]
- [[.unnormalize()]] - `defined_in` [EXTRACTED]
- [[ActionCodec]] - `contains` [EXTRACTED]
- [[ActionCodec (OpenVLA token decode)]] - `defined_in` [EXTRACTED]
- [[Build a codec from a fetched ``dataset_statistics`` mapping.          Mirrors Op]] - `defined_in` [EXTRACTED]
- [[Decode OpenVLA discrete action tokens to continuous actions.      Immutable (cod]] - `defined_in` [EXTRACTED]
- [[Decode a full action's token ids to a continuous action vector.          Args]] - `defined_in` [EXTRACTED]
- [[Dimensionality of the action space (= number of quantile entries).]] - `defined_in` [EXTRACTED]
- [[Map a single action token id to a bin-centre index in ``0, n_bins-2``.]] - `defined_in` [EXTRACTED]
- [[OpenVLA action codec discrete action token ids - continuous 7-DoF (Task 3).  T]] - `rationale_for` [EXTRACTED]
- [[Return the normalised value (bin centre, in ``-1, 1``) for a bin index.]] - `defined_in` [EXTRACTED]
- [[The ``n_bins - 1`` bin centres (what de-tokenised tokens map to).]] - `defined_in` [EXTRACTED]
- [[The ``n_bins`` uniform bin edges over ``-1, 1``.]] - `defined_in` [EXTRACTED]
- [[Un-normalise per-dim normalised actions using q01q99 under ``mask``.          `]] - `defined_in` [EXTRACTED]
- [[ndarray_7]] - `defined_in` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/OpenVLA_Action_Codec

## 📄 Source

`src/evasion_tax/policy/action_codec.py`
