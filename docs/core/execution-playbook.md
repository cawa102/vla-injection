...
- [ ] **GCG micro-benchmark** on the **registered A5000** (s/target + peak VRAM + max candidate-batch at 24 GB; published H100 timings do NOT transfer) → resolve D4 + D7. **＋ (Codex #2 #5) micro-bench L1 extraction + adaptive-GCG-against-probe → decide whether the adaptive-L1 arm is in scope at the measured budget.**
