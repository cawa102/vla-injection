...
- **Last updated:** 2026-07-03 (**Execution host update:** current work is on the GPU-equipped CSB PC; there is no current "no GPU here" limitation. OpenVLA/GCG/LIBERO work may run directly on the registered A5000 when the playbook calls for it, with exact HW/env logging and no cross-HW mixing.)
