# T7 — Codex-#2 model-free hooks + §4b-III cross-layer eval (implementation plan)

> Companion to the **execution playbook** (`docs/core/t7-execution-playbook.md` §4b, §5, §7 task ledger) and
...
**Done = verified when:** new modules + tests land, `pytest tests/t7` green (312 + new), `ruff check src tests`
...
**Config (pinned, logged with every run):** add to `DetectorConfig` (`src/t7/config/schema.py`):
...
**New module `src/t7/eval/power.py`** (pure, no model):
...
**Tests** (`tests/t7/eval/test_power.py`): `required_benign_n(0.01)==300`, `==60` at 0.05; powered/underpowered
...
**New module `src/t7/metric/coverage.py`** (mirrors the frozen schema scope; no heavy imports → both `attack`
...
**Tests** (`tests/t7/metric/test_coverage.py`): each kind → correct status; supported-kind + unresolvable
...
**New module `src/t7/eval/cross_layer.py`** (pure stats; reuses `attack/frontier.py` geometry):
...
**Tests** (`tests/t7/eval/test_cross_layer.py`): `frontier_from_outcomes` reproduces a hand-aggregated frontier;
...
- Verify: `.venv/bin/python -m pytest tests/t7` (green) · `.venv/bin/ruff check src tests` (clean) ·
  `pyright` (clean).
