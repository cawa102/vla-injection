...
- **Currently:** Executing the **pre-GB10 local-prep plan** (`docs/plans/t7-local-prep-plan.md`) on M1/8 GB via TDD. **Done 2026-05-31: Tasks 0,1,2,3,4,6,7** — env scaffold, repro infra, data records, **action codec (OpenVLA formula verified from source `c8f03f48`; provenance recorded)**, privileged-state adapter, FP-calibrated detector, eval harness (ROC/AUC, TPR@FPR + Wilson/CP CIs, latency, split-disjointness) — **162 tests green, full `src/t7` type-clean** (see plan Status table). **Remaining delegable:** Task 8 baselines, Task 9 configs/scripts/figures, Task 11 runbook. **Needs author/me first:** **Task 5 metric-(A) schema freeze** (load-bearing — author design review); Task 10 LIBERO smoke (time-boxed).
