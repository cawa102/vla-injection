...
- Record fields to add (all **required**, not optional — Codex R1): `target_tier`, `asr_frame ∈ {"action","world"}`, `reached_single_frame` (bool); for **Tier B**: `approach_asr` (EE region-entry, headline) + `manipulation_asr` (**logged diagnostic only**, Codex R2) + `metric_a_p2_ablated_per_step` (detector-independent L2, Task 5) — never claim "picked the wrong object" from approach alone. Plus `distractor_object` / `adv_instruction` (semantic only), existing `rollout_asr_reached`, folded `cost`, and `loss_history` (`run_gcg`'s per-step best-so-far GCG loss, non-increasing — logged so the optimisation curve is regenerable from the write-once record, not just the final `best_loss`; the redesigned targets should drive it below the anchor pilot's ~6.6 plateau). At run time, **assert `distractor_object` is in `object_poses` after reset** (Task 3 runtime validation).
