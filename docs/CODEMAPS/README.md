...
# CODEMAPS — T7 (Embodiment Evasion Tax)

Token-lean architecture map for AI context. MSc research code, **not** an app: no HTTP API, no UI, no DB.
Python package `src/t7` (model-free, unit-tested on M1/8 GB) + CLI `scripts/` (GPU-node-only runs).
Source of truth for *what & why* = `docs/core/t7-execution-playbook.md`; these maps cover *structure*.
...
- Scanned: 36 `src/t7` `.py` (27 modules · 9 `__init__`, ~4.3k LOC) · 9 `scripts` · 31 files in `tests/`
...
- Regenerate after each milestone (M-tag) or when `src/t7` subpackages change.
