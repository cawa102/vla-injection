...
- [x] **2. Run the existing model-free test suite (395 tests)** — *verify:* parity with the Mac (same pass count) — *2026-06-17 box: all green (after restoring `.git` via real clone — see `ssh.md` §5/§6)*
  - **GPU-safety note (2026-07-03):** the full suite is now safe to run **on the box** — previously three tests assumed a CUDA-free host (author's mac). The worst, `test_main_exits_with_gpu_required_when_no_cuda`, did **not** mock `cuda_available`, so on a real GPU box its no-CUDA guard fell through and `bench.main()` ran the **actual ~2 h early-stop bench** on the A5000 (caught via `--durations`; a unit test must never do GPU work). Fixed by mocking `cuda_available → False` in the guard test and subprocess-isolating the `*_imports_without_torch` clean-import checks (commits `2dfa053` / `2d747aa`). A static scan confirmed **every** `.main()` test on a cuda-guarded script now mocks the guard → `pytest` here does **zero GPU work**.
