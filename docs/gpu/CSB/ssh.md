...
- **`huggingface-hub` は lock内・上限ピンが必要** — `huggingface-hub` は model-free 直接依存(stats fetcher が使う)。旧 pyproject の `>=0.20`(上限なし)を `uv.lock` が **1.17.0** に解決していたため、`uv pip install transformers==4.40.1`(`hub<1.0` 要求)で 0.36.2 に下げても **`uv run`/`uv sync` のたびに lock の 1.17.0 へ戻され** transformers が `ImportError: huggingface-hub<1.0 required` で落ちた(numpy が 2.x に戻っていたのと同型)。**torch と違い lock外ではなく lock不整合** → `pyproject`+`uv.lock` に `huggingface-hub>=0.20,<1.0` を固定済み(→ hub 0.36.2)。**`--no-sync` は対症療法なので使わない**;箱では `git pull` 後に **`uv sync --inexact`**(hub を lock に合わせつつ lock外 GPU スタックは残す)。
- **`uv lock`/`uv sync` の連打で numpy が壊れることがある** — lock 再生成の churn で numpy が**不完全展開**(`numpy/testing/` 欠落 → `np.testing.assert_array_equal` が消え、`numpy.testing.__file__ is None`)になった事例。バージョンは 1.26.4 のまま。`uv sync --reinstall-package numpy` で復旧。
